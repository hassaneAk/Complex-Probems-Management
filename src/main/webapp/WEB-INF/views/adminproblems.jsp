	...
	<body class="whiteBody">
		...
		<c:choose>
			<c:when test="${archive==true }">
				<form  class="search" action="searchArchiProblem" method="post">
					<input type="text" name="keyword" id="keyword" placeholder="Chercher un problème dans le repertoire en commun.." required="required">
		...
		<div class="centeredDiv">
			<div class="leftDiv" style="margin:40px; padding:0;">
				...
				<!-- Liste des problèmes -->
				<div class="divReplyTitle">
					<h2>${problems.size() } problèmes <c:if test="${archive==true }">archivés</c:if> </h2>
				</div>
								...
								<c:when test="${archive==true }">
									<div class="reply-footer" style="border-top: 1px solid #80bfff;">
										<p class="left" style="color: #80bfff; padding-left: 10px; ">Accéder au :
											<a style="font-size: 15px;" href="sprOfArchiPr?idPr=${pr.idPr}"> Sous Problèmes </a> --
											<a style="font-size: 15px;" href="repOfArchiPr?idPr=${pr.idPr}">Réponses</a>
										</p>
									</div>
								</c:when>
								<c:otherwise>
