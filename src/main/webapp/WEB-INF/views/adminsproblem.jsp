		...
		<div class="centeredDiv">
			...
			<!-- Problème -->
				<div class="divReplyTitle" style="margin-top:40px; ">
					<h2> Problème <c:if test="${archive==true }">archivé</c:if> sélectionné  </h2>
				</div>
							...
							<c:when test="${archive==true }">
								<div class="reply-footer" style="border-top: 1px solid #80bfff;">
									<p class="left" style="color: #80bfff; padding-left: 10px; ">Accéder au :
										<a style="font-size: 15px;" href="repOfArchiPr?idPr=${problem.idPr}">Réponses</a>
									</p>
								</div>
							</c:when>
							<c:otherwise>
				...
				<!-- Sous problèmes -->
				<div class="divReplyTitle">
					<h2>${sproblems.size()} sous problèmes <c:if test="${archive==true }">archivés</c:if> </h2>
				</div>
